...
import torch
import pyttsx3
from aiven_db_tables import details_for_money_request, fetch_and_print_inventory, update_money_table, get_employee_performance, get_project_details, insert_interview_details, get_client_query_details
...
def chatbot(engine,input_text, qa_pipeline):
    ...
    if prediction == 0:
        ...
        elif amount==None or str(amount).lower() == None or str(amount).lower() == "amount" or str(amount).lower() == "money":
        ...
        if project_details:
                print({money_request_dict["amount"]})
                ...
                if "yes" in new_query.lower():
                    ...
                    if money_request_dict["reason"] is not None:
                        updated_reason = current_reason + " " + str(money_request_dict["reason"]) 
                    else:     
        ...
        while missing_fields:
            ...
            if "yes"  in new_input.lower() or "do it " in new_input.lower() or "want to " in new_input.lower():
