...
db_params = {
    'dbname': os.getenv("dbname"),  # Aiven DB name
    'user': os.getenv("user"),          # Aiven DB user
    'password': os.getenv("password"),  # Aiven DB password
    'host': os.getenv("host"),          # Aiven DB host (this is usually something like your_project_name.aivencloud.com)
    'port': os.getenv("port")                   # Aiven PostgreSQL default port (can be different if specified)
}
